...
namespace MusicStore.Models
{
    public static class SampleData
    {
        ...
        private static MusicStoreContext context = null;
        ...
        public static async Task InitializeMusicStoreDatabaseAsync(IServiceProvider serviceProvider, bool createUsers = true)
        {
            ...
            {
                context = serviceScope.ServiceProvider.GetService<MusicStoreContext>();
        ...
        private static Album[] GetAlbums(string imgUrl, Dictionary<string, Genre> genres, Dictionary<string, Artist> artists)
        {
            ...
            {
                var albums = new Album[context.Albums.Count()];

                var genresDB = context.Genres.ToList();
                var artistDB = context.Artists.ToList();

                int i = 0;
                foreach (var alb in context.Albums)
                {
                    albums[i] = new Album
                    {
                        Title = alb.Title,
                        Genre = genresDB.Where(obj => obj.GenreId == alb.GenreId).First(),
                        Price = alb.Price,
                        Artist = artistDB.Where(obj => obj.ArtistId == alb.ArtistId).First(),
            ...
            get
            {
                if (artists == null)
                {
                    ...
                    else
                    {
                        ...
                        artists = new Dictionary<string, Artist>();
                        foreach (var art in context.Artists)
                        {
                            artists.Add(art.Name, art);
                        }
                    }
                }
